...
| P0#4/P0#5/P0#6 (this commit) | **Concurrency, restart recovery, safe fallback** — the last three open P0s. (P0#4, INV-010) `claim_queued_task()` in `orchestrator.py`: the concurrency guard and the `QUEUED -> RUNNING` transition are one conditional UPDATE (cap subquery in the WHERE), shared by the orchestrator and the RQ worker — the cap is enforced at the single claim seam, not merely calculated. (P0#5, INV-011) `drive_session()` now calls `recover_orphans()` first, so a restarted cloud dyno reclaims RUNNING work with expired leases instead of stranding it. (P0#6, INV-014) the unplannable-goal fallback re-runs the high-risk classifier on the raw goal (`fallback_plan`: "rotate production credentials" degrades to HIGH, never LOW) and `drive_session` reports `unfinished` > 0 when the bounded drive stops with work left, with the Telegram summary saying so. 8 adversarial tests in `tests/adversarial/test_audit_p0_completion.py`; suite at **1015 passed**, ruff/format/strict-mypy clean. |
